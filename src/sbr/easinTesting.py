...
class RestEntity:
    def __init__(
        self,
        baseurl="http://localhost:8080/serverTest/0.0.1-SNAP/api/",
        entity="",
        secured=False,
    ):
    ...
    def GetOne(self, id):
        ...
        return requests.get(
            self.baseurl + self.entity + "/" + id, headers=self.headers
        )
    ...
    def ExportOne(self, id):
        ...
        print(self.entity + "  Getted .. \t" + id)
        return requests.get(
            self.baseurl + self.entity + "/" + id + "/export",
            headers=self.headers,
        )
    ...
    def GetAll(self):
        ...
        return requests.get(
            self.baseurl + self.entity + "/all", headers=self.headers
        )
    ...
    def Auth(self, email, passwd):
        ...
        print(">>   Auth query ", da)
        query = requests.post(
            self.baseurl + "auth/token", data=da, headers=self.nsHeaders
        )
...
class TestCrudUser(unittest.TestCase):
    def test_ANewUSerCreate(self):
        self.tester = RestEntity(
            "http://localhost:8080/serverTest/0.0.1-SNAP/api/", "user", True
        )
        ...
        query = self.tester.Create(
            ...
            + '","email":"'
            + self.userEmail1
            + '","activated":true,"langKey":"EN","imageUrl":"L0evg9vxAQ","resetDate":null,"id":null}'
        )
